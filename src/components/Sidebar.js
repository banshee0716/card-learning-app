...
const Sidebar = ({ isOpen }) => {
  ...
  const { topics, currentTopic, setCurrentTopic, addTopic, deleteTopic } =
    useFlashcards();
    ...
    <nav className={`sidebar ${isOpen ? "open" : "closed"}`}>
      ...
      <ul>
        {topics.map((topic, index) => (
          <li key={index} className={currentTopic === topic ? "active" : ""}>
            <button onClick={() => handleTopicChange(topic)}>{topic}</button>
            {/* 只有當有多於一個主題時才顯示刪除按鈕 */}
            {topics.length > 1 && (
              <button
      ...
      <form onSubmit={handleAddTopic} className="add-topic-form">
        <input
          ...
          aria-label="新增主題"
        />
        <button type="submit" aria-label="添加新主題">
          +
        </button>
      </form>
    </nav>
